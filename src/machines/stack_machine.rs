        ...
        match expr {
            Term::Application {
                ...
                if location == Location::Output {
                    self.push_output(argument.to_string());
                } else {
            ...
            Term::Abstraction {
                ...
                match self.get_from(&location) {
                    // (S_A ; S_a·N , a⟨x⟩.M, K)  →  (S_A ; S_a , {N/x}M, K)
                    Some(argument) => {
                        let term = if binds.ends_with("_entry") {
                            term.freshen_locations()
                        } else {
                            *term
                        };

                        self.expression = term.substitute(binds.as_str(), argument);
            ...
            Term::Variable { .. } => StepResult::Failure,
            ...
            Term::Loop { term, branch } => {
                //(S_A , M^i , K)  →  (S_A , M , (i → M^i)K)

                let current_term = Term::Loop {
                    term: term.clone(),
                    branch: branch.clone(),
                };

                self.expression = *term;
                self.continuation_stack.push((branch, current_term));
            ...
            Term::Special(special) => match special {
                ...
                Special::IntCast => {
                    if let Some(number) = self.get_from(&Location::Main).and_then(term_to_number) {
                        self.memory
                            .entry(Location::Main)
                            .or_default()
...
impl StackMachine {
    ...
    fn get_bool(&mut self) -> Option<bool> {
        let bool = self.get_from(&Location::Main);
        if let Some(Term::Choice(Choice::Constant(Constant::Boolean(bool)))) = bool {
            return Some(bool);
        }
        None
    }
    ...
    fn get_two_values(&mut self) -> Option<(Term, Term)> {
        let second = self.get_from(&Location::Main);
        if let Some(second) = second {
            let first = self.get_from(&Location::Main);
    ...
    fn get_two_nums(&mut self) -> Option<(Number, Number)> {
        let second = self.get_from(&Location::Main);
        if let Some(second_num) = second.and_then(term_to_number) {
            let first = self.get_from(&Location::Main);

            if let Some(first_num) = first.and_then(term_to_number) {
    ...
    fn get_two_bools(&mut self) -> Option<(bool, bool)> {
        let second = self.get_from(&Location::Main);
        if let Some(Term::Choice(Choice::Constant(Constant::Boolean(second_bool)))) = second {
            let first = self.get_from(&Location::Main);

            if let Some(Term::Choice(Choice::Constant(Constant::Boolean(first_bool)))) = first {
                return Some((first_bool, second_bool));
            }
        }
        None
    }
...
mod tests {
    ...
    fn run_returns_terminal_step_result_and_captures_output() {
        let mut machine = StackMachine::new(Term::Application {
